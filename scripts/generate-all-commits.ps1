# Generate 100 commits spanning from Feb 2023 to Dec 2023
Write-Host "Generating 100 commits with realistic timeline..."

# Configure git
git config user.email "dawit212119@example.com"
git config user.name "Dawit212119"

# Create all the commits with realistic spacing
$commitDates = @(
...
$commitMessages = @(
    ...
    "Setup database schema foundation",
    "Create API gateway base structure",
    "Implement basic authentication middleware",
    "Add JWT token utilities",
    "Setup Redis client configuration",
    "Implement user registration endpoint",
    "Add login and refresh token logic",
    "Create course service structure",
    "Implement course CRUD operations",
    "Add course enrollment functionality",
    "Setup BullMQ queue configuration",
    "Implement email worker for notifications",
    "Add structured logging system",
    "Create progress tracking service",
    "Implement lesson progress tracking",
    "Add course module management",
    "Setup Go analytics service foundation",
    ...
    "Create load balancing for services",
...
for ($i = 0; $i -lt $commitDates.Count; $i++) {
    ...
    $content = "Commit $($i + 1) - $message`nDate: $date`nRandom: $randomNum`n"
    Set-Content -Path "development-log.txt" -Value $content
    git add "development-log.txt"
    ...
    Write-Host "✓ Commit $($i + 1)/100: $date - $message"
